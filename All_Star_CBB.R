...
# 1. Load datasets
# ============================================================
CBBPlayers <- read_excel("/Users/naeem/OneDrive/Documents/GitHub/SAP_CBB_AllStar/CBBPlayers_with_Positions.xlsx")

# ============================================================
# 2. All-Star Lookup Table
# ============================================================

# Manual list (name + team just for matching)
manual_allstars <- tribble(
...
# Convert manual list to PID-based lookup
allstars_lookup <- manual_allstars %>%
  left_join(
    CBBPlayers %>%
      select(pid, player_name, team) %>%
      distinct(),
    by = c("player_name", "team")
  ) %>%
  select(pid, start_year, end_year)

# Safety check
if (any(is.na(allstars_lookup$pid))) {
  stop("Some All-Stars did not match to a pid. Fix before continuing.")
}
...
CBBPlayers_labeled <- CBBPlayers %>%
  left_join(allstars_lookup, by = "pid") %>%
  mutate(
    is_all_star = ifelse(
      !is.na(start_year) &
        year >= start_year &
...
CBBPlayers_labeled <- CBBPlayers_labeled %>%
  select(pid, everything())

# Save to CSV
write.csv(CBBPlayers_labeled, 
          "C:/Users/naeem/OneDrive/Documents/CBB_labeled.csv", 
          row.names = FALSE)